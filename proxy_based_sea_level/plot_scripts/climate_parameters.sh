...
gmt begin climate_parameters pdf,png
    ...
    awk -v ymin=${ymin} -v ymax=${ymax} 'BEGIN{counter=1}{if (counter==NR) {printf "%s\n%i %4.1f\n%i %4.1f\n%i %4.1f\n%i %4.1f\n", ">",$1/1000, ymax, $2/1000, ymax, $2/1000, ymin, $1/1000, ymin; counter = counter + 2 } }' ../age_boundaries/lr04_MIS.txt > temp/warm_mis.txt

    awk -v ymin=${ymin} -v ymax=${ymax} 'BEGIN{counter=2}{if (counter==NR) {printf "%s\n%i %4.1f\n%i %4.1f\n%i %4.1f\n%i %4.1f\n", ">",$1/1000, ymax, $2/1000, ymax, $2/1000, ymin, $1/1000, ymin; counter = counter + 2 } }' ../age_boundaries/lr04_MIS.txt > temp/cold_mis.txt
	...
	awk -F'\t' '{print $1/1000, $2}' ../18O/lr04.txt > temp/lr04.txt
...
# CO2
	...
	gmt psxy  temp/cold_mis.txt   -L -Glightskyblue ${J_options} ${R_options} 
	awk -F'\t' '{print $1/1000, $2}' ../CO2/Bereiter_etal_2015_Antarctica_Composite.txt > temp/antarctica_CO2.txt
...
END_TEXT
	...
	awk -F'\t' '{print $1/1000, $2}' ../sea_level/Rohling_etal_2022_LR04_process_original.txt > temp/sea_level.txt
...
END_TEXT
	...
	awk -F'\t' '{print $1/1000, $2}' ../insolation/65N_June_21.txt > temp/Daily_insolation65.txt
